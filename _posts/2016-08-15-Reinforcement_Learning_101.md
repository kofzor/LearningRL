...
At the beginning of interaction with an MDP, an agent observes an initial state $$s_0$$ and chooses an action $$a_0$$. From that point on, at each discrete timestep $$t = 0, 1, 2, ...$$, it observes a successor state $$s_{t+1}$$ and a reward $$r_{t+1}$$, and chooses a new actions.
